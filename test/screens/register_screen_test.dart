...
void main() {
  ...
  Future<bool> hasValidationErrors(WidgetTester tester) async {
    ...
    final errorKeywords = [
      'nhập', 'Vui lòng', 'không hợp lệ', 'không khớp', 
      'phải có', 'ít nhất', 'Email', 'mật khẩu', 'xác nhận'
    ];
  ...
  group('RegisterScreen - Edge Cases', () {
    ...
    testWidgets('register failure shows error message', (tester) async {
      ...
      when(mockAuthProvider.register(
        ...
        password: anyNamed('password'),
        name: anyNamed('name'),
      )).thenAnswer((_) async => {'success': false, 'message': 'Email đã tồn tại'});
      ...
      expect(errorTexts.evaluate().isNotEmpty || errorMessages.evaluate().isNotEmpty, isTrue);
    });
    ...
    testWidgets('can enter text in all fields', (tester) async {
      ...
      await tester.enterText(find.byType(TextFormField).at(1), 'nguyenvana@example.com');
      await tester.enterText(find.byType(TextFormField).at(2), 'password123');
